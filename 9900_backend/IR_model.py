...
def ir_model(query):
    ...
    corpus_sorted, features, idf, vocabulary, length_corpus, chunks_corpus_raw, title_corpus_raw = keywords_extracted_tfidf()
    ...
    for j in range(len(corpus_sorted)):
        ...
        for i in range(len(query_tfidf)):
            sum_multi += query_tfidf[i] * chunk_tfidf[i]
    #    if sqrt != 0:
...
print(ir_model('what is the problem of cross entropy')      )
print(ir_model('tell me something about Artificial Neural Networks')      )  
print(ir_model('Weight Decay')      )
